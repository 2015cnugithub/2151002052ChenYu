...
public:
    ...
    ~HourlyWorker() {delete name;cout<<"Deleting HourlyWorker."<<endl;}
...
public:
    ...
    Worker(name_val,age_val,sex_val,pay_per_hour_val) {}
    ~SalariedWorker() {delete name;cout<<"Deleting SalariedWorker."<<endl;}
    void Compute_pay(double hours);
};
...
int main()
{
	...
	cin>>per_pay;
	cout<<endl;
	if(per_pay==30||per_pay==50) *(workers+i)=new SalariedWorker(name,age,sex,per_pay);
        else *(workers+i)=new HourlyWorker(name,age,sex,per_pay);
    }
    do{
        cout<<"工人菜单\n";
        for(int i=0;i<5;i++)
